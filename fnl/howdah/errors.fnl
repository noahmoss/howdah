...
(fn errors.format [err sql sql-start]
  "Formats a SQL error as lines for the results buffer. sql-start is the zero-based
  row and byte column where sql begins in the query buffer."
  (-> (vim.iter [(headline err)
                 (detail-lines err)
                 (buffer-query-lines err sql sql-start)
...
(fn buffer-position [sql position sql-start]
  ...
  (let [{: row : char-col : text} (locate sql position)
        ...
        sql-start-byte-col (if (= row 0) sql-start.byte-col 0)]
    {:lnum (+ sql-start.row row) :col (+ sql-start-byte-col byte-col)}))

(fn diagnostic [err sql sql-start]
  "A vim.diagnostic entry for the error's position in the query buffer."
  (let [{: lnum : col} (buffer-position sql err.position sql-start)]
    {: lnum
     : col
     :message err.message
...
(fn errors.set-diagnostic [buffer err sql sql-start]
  "Marks the error position on the query buffer, if the error has one."
  (when err.position
    (vim.diagnostic.set diagnostics-ns buffer [(diagnostic err sql sql-start)])))
